            ...
            if(der!=null){ 
                ...
                if(TablaSimbolos.yaDeclaradaConNiv(id)){
                    ...
                    if(izquierda.tipo().equals(Tipo.ARRAYUNIDIM)&&derecha.tipo().equals(Tipo.ARRAYUNIDIM)){
                        ...
                        this.tipo=derecha;
                    }else if(izquierda.tipo().equals(Tipo.ARRAYMULTIDIM)&&derecha.tipo().equals(Tipo.ARRAYMULTIDIM)){
                        if(izquierda.getSubtipo().equals(derecha.getSubtipo())){
                            if(izquierda.getLongitud()>=derecha.getLongitud()){
                            Generador.asignacion(id,((Expresion)der).getPalabra());
                            }else{
                        ...
                        tipo=derecha;
                    }else if(izquierda.tipo().equals(Tipo.STRING)&&derecha.tipo().equals(Tipo.CHAR)){
                        
                        String t0=Generador.nuevaTemp();
                        Generador.asignacion(t0,((Expresion)der).getPalabra());
                        Generador.asignacion(id+"["+"0"+"]",t0);
                       
                        Generador.asignacion(id+"_length","1" );
                        tipo=izquierda;
                    }else{
