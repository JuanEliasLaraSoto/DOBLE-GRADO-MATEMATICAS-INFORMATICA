        ...
        public void generarCTD(){
            if(der!=null){ 
                ...
                if(TablaSimbolos.yaDeclaradaConNiv(id)){
                    ...
                    if(izquierda.tipo().equals(Tipo.ARRAYUNIDIM)&&derecha.tipo().equals(Tipo.ARRAYUNIDIM)){
                    ...
                    }else if(izquierda.tipo().equals(Tipo.SET)&&derecha.tipo().equals(Tipo.SET)){
                        if(izquierda.getSubtipo().equals(derecha.getSubtipo())){
                        ...
                        Generador.asignacion(id+"["+i+"]", t0);
                        Generador.asignacion(i,i+"+1");
                        Generador.salto(aux);
                        Generador.etiq(f);
                        Generador.asignacion(id+"_length",((Expresion)der).getPalabra()+"_length");
                        this.tipo=derecha;
                    }else{
                        Generador.error("tipos diferentes");
                    }

                    }else if(izquierda.tipo().equals(Tipo.SET)&&derecha.tipo().equals(Tipo.ARRAYUNIDIM)){
                        if(izquierda.getSubtipo().equals(derecha.getSubtipo())){
                        String v=Generador.nuevaLabel();
                        String f=Generador.nuevaLabel();
                        String aux=Generador.nuevaLabel();
                        ...
                        Generador.asignacion(i,"0");
                        Generador.etiq(aux);
                        Generador.comparacion(i, "<", ((Expresion)der).getTipo().getLongitud()+"", new DosEtiq(v, f));
                        Generador.etiq(v);
                        Generador.asignacion(t0, ((Expresion)der).getPalabra()+"["+i+"]");
                        Generador.asignacion(id+"["+i+"]", t0);
                        Generador.asignacion(i,i+"+1");
                        Generador.salto(aux);
                        Generador.etiq(f);
                        Generador.asignacion(id+"_length",((Expresion)der).getTipo().getLongitud()+"");
                        this.tipo=izquierda;
                        }else{
                            Generador.error("tipos diferentes");
                        }
